...
class TerrainBuilder:
    ...
    def base_terrain_snippet(self, size_m: float = 40.0) -> str:
        """Create the single shared Terrain mesh."""
        return (
            "import bpy, bmesh\n"
            "# Remove any existing flat planes that would stack\n"
            "for _o in list(bpy.data.objects):\n"
            "    if _o.type == 'MESH' and _o.name.startswith('Plane'):\n"
            "        bpy.data.objects.remove(_o, do_unlink=True)\n\n"
            "bpy.ops.mesh.primitive_plane_add(size=1, location=(0,0,0))\n"
            "_terrain = bpy.context.active_object\n"
            "_terrain.name = 'Terrain'\n"
            f"_terrain.scale = ({size_m/2}, {size_m/2}, 1)\n"
